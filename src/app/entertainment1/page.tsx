...
import { getEntertainment1Data } from '@/lib/entertainment1.client';
...
function Entertainment1PageClient() {
  ...
  const SCROLL_STORAGE_KEY = 'entertainment1_scroll';
  ...
  const loadInitialData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getEntertainment1Data(getRequestParams(0));
    ...
    if (currentPage > 0) {
      ...
      const fetchMoreData = async () => {
        try {
          ...
          const result = await getEntertainment1Data(
  ...
  return (
    <PageLayout activePath="/entertainment1">
...
export default function Entertainment1Page() {
  return (
    <Suspense>
      <Entertainment1PageClient />
    </Suspense>
  );
}
